...
class Tp extends Taglib
{
    ...
    public function tagMenu($tag)
    {
        ...
        $menu = isset($tag['menu']) ?
            ...
            ['add', 'forbid', 'resume', 'delete', 'recyclebin','batchadd'];
        ...
        foreach ($menu as $k => $m) {
            ...
            switch ($m) {
                case 'add':
                    ...
                    break;
				 case 'batchadd'://批量新建
                    $title = isset($titleArr[$k]) && $titleArr[$k] ? $titleArr[$k] : '批量添加';
					$title='批量添加';
                    list($url, $param) = $this->parseUrl($url);
                    $parseStr .= '<a class="btn btn-primary radius mr-5" href="javascript:;" onclick="layer_open(\'' . $title . '\',\'<?php echo \think\Url::build(\'' . $url . '\', [' . $param . ']); ?>\')"><i class="Hui-iconfont">&#xe600;</i> ' . $title . '</a>';
                    break;
                case 'forbid':
                    ...
                    list($url, $param) = $this->parseUrl($url);
                    $parseStr .= '<a href="javascript:;" onclick="forbid_all(\'<?php echo \think\Url::build(\'' . $url . '\', [' . $param . ']); ?>\')" class="btn btn-warning radius "><i class="Hui-iconfont">&#xe631;</i> ' . $title . '</a>';
                    break;
                case 'resume':
                ...
                case 'sedit':
                    ...
                    break;
				case 'xcopy':
                    $title = isset($titleArr[$k]) && $titleArr[$k] ? $titleArr[$k] : '复制';
                    list($url, $param) = $this->parseUrl($url, 'id=$vo["id"]');
                    $parseStr .= ' <a title="' . $title . '" href="javascript:;" onclick="layer_open(\'' . $title . '\',\'<?php echo \think\Url::build(\'' . $url . '\', [' . $param . ']); ?>\')" style="text-decoration:none" class="ml-5">复制</a>';
                ...
                case 'sdeleteforever':
				 ...
				 case 'more':
					...
					$title = isset($titleArr[$k]) && $titleArr[$k] ? $titleArr[$k] : '更多';
