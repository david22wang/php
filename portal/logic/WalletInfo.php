...
class WalletInfo
{
	...
	public function freeze($client_info_id,$amount,$csum,$moneytp,$fid,$walletid=3,$remark='前台')
	{//冻结
		//支出最后一个参数walletid 没有使用用
		//$client_info_id=Session::get('client_info_id');
		...
		//$moneytp = $this->request->param('moneytp');
		//$fid=0;
		$data=['remark'=>$remark,'amount'=>$amount,'money_tp'=>$moneytp,'clientid'=>$client_info_id,'fid'=>$fid,'status'=>'0','totalamount'=>'0','trade_type'=>'-2','walletid'=>$walletid];
		//$walletid=3;
		$flag=Loader::model('ClientwalletDetail')->updateWallet_freeze($data,$walletid);//冻结
     ...
     public function unfreeze($client_info_id,$amount,$csum,$moneytp,$fid,$walletid=3,$remark='前台')
	{//收入最后一个参数walletid 没有使用用
		//$client_info_id=Session::get('client_info_id');
		...
		$data=['remark'=>$remark,'amount'=>$amount,'money_tp'=>$moneytp,'clientid'=>$client_info_id,'fid'=>$fid,'status'=>'0','totalamount'=>'0','trade_type'=>'2','walletid'=>$walletid];
		//$walletid=3;
		$flag=Loader::model('ClientwalletDetail')->updateWallet_unfreeze($data,$walletid);//解冻
